...
4. ~~**Benchmark rigor** — numbers recorded from a single `-benchtime=100ms`~~ done (done — benchstat protocol recorded in bench_test.go doc (seam session M4, 2026-09-16))
   ~~run on one machine. Directionally solid (quadratic scaling is~~
   ~~unambiguous) but not a benchstat-grade baseline. Effort: S/M.~~

## c) NOT STARTED

1. ~~**v0.2.0 release** — owner-gated; checklist fully prepared (M13).~~ done (executed 2026-09-16 — v0.2.0 cut (805aeba; owner package annotated inline))
...
| ~~2~~  | ~~Owner: approve v0.2.0 cut (package Q2)~~ done — approved and executed — v0.2.0 cut 2026-09-16 (805aeba)                                                                                                                                  | ~~Critical~~ | ~~S~~  | ~~Decision~~      |
| ~~3~~  | ~~Execute the v0.2.0 go-release checklist on approval (tag, proxy, pkg.go.dev, CV bump)~~ done — executed 2026-09-16 — checklist run recorded in the owner package (tag 805aeba; release.yml extraction bug fixed 350b815; release manual) | ~~Critical~~ | ~~M~~  | ~~Release~~       |
| ~~4~~  | ~~Document the identical-text → single-vector rule on `Build` (doc comment)~~ done — Build doc comment states the rule (build.go:99-104, seam session F3.1)                                                                                | ~~High~~     | ~~S~~  | ~~Documentation~~ |
| ~~5~~  | ~~Add tag-protection rule before v0.2.0 (`gh api …/tags/protection`)~~ done — protect-tags ruleset id 23541172 active (seam session F5.2)                                                                                                  | ~~High~~     | ~~S~~  | ~~Security~~      |
...
| ~~8~~  | ~~Re-run benchmarks with benchstat protocol; update reference numbers~~ done — benchstat protocol recorded in bench_test.go doc (seam session M4)                                                                                          | ~~Medium~~   | ~~M~~  | ~~Quality~~       |
| ~~9~~  | ~~Set `MinScore` in README snippet / examples to avoid 0.00 hits~~ done — README snippet sets MinScore 0.25 (docs-health pass 2026-09-16 02-10)                                                                                            | ~~Low~~      | ~~S~~  | ~~Documentation~~ |
...
| ~~12~~ | ~~Add `ExampleNewStore` — store.go is the only major file without an example~~ done — as ExampleOpenStore (rename: no NewStore identifier exists; example_test.go:141, seam session F3.2)                                                  | ~~Medium~~   | ~~S~~  | ~~Documentation~~ |
| ~~13~~ | ~~Add a `SearcherOptions` (ReferenceKind/DocumentKinds) example variant~~ done — ExampleNewSearcherWithOptions (example_test.go:190, seam session F3.3)                                                                                    | ~~Medium~~   | ~~S~~  | ~~Documentation~~ |
| ~~14~~ | ~~Store round-trip benchmark (persist + snapshot load at 1k/10k nodes)~~ done — BenchmarkStoreRoundTrip 1k/10k in bench_test.go (seam session F4.4)                                                                                        | ~~Medium~~   | ~~M~~  | ~~Quality~~       |
| ~~15~~ | ~~Cross-link the measured quadratic cost from ROADMAP's 50k note~~ done — ROADMAP cites the measured ~370ms/1000-doc cost (seam session M4)                                                                                                | ~~Low~~      | ~~S~~  | ~~Documentation~~ |
...
| ~~19~~ | ~~Run `qmd embed` (173 CV docs need embedding — noticed in the MCP banner)~~ done — 1677 chunks / 173 docs embedded; needs-embedding 0 (seam session F8.2)                                                                                 | ~~Low~~      | ~~S~~  | ~~Tooling~~       |
| 20     | `lsp_restart golangci_lint_ls` after daemon commits (habit)                                                                                                                                                                                | Low          | S      | Tooling           |
| ~~21~~ | ~~GitHub Release automation (workflow) as v0.2.0 follow-up~~ done — release.yml drafted + actionlint-clean (seam session F9)                                                                                                               | ~~Low~~      | ~~M~~  | ~~Release~~       |
