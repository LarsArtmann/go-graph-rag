...
| Make concurrent embed batches available (`EmbeddingConfig.EmbedConcurrency`, default 1) | 🔴 `TODO`    | Med    | M      | cold builds pay RTT × batch-count serially (`embed_openai.go:163-172`); bounded stdlib worker pool, opt-in, preserves order; review: `docs/architecture-understanding/2026-09-19_13-30_architecture-review.html` (R4)                                                                                                                      |
| SDK hardening batch: cache-error policy + Searcher aliasing + single-hash               | 🔴 `TODO`    | Med    | S      | lookupCache swallows read errors while writes are fatal (`build.go:206-217` vs `:190-194`, owner-taste policy call); NewSearcherWithOptions retains caller map/slice despite the immutability promise (`search.go:108-115`); resolveVectors double-hashes each miss (`build.go:163,197`); review findings f-cache/f-aliasing/f-double-hash |
