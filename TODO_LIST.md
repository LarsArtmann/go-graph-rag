...
| Design the pluggable store/search seam (scenario B)             | 🔴 `TODO`    | High   | M      | owner decision 2026-09-15: DESIGN NOW (dgraph status §g ANSWERED); deliverable per owner 2026-09-16: ADR + Go interface sketch, binding before code; owner 2026-09-16: next work session; CGO acceptable in optional adapters only, core stays pure-Go; `docs/research/2026-09-15_dgraph-adoption.md` §6/§8; encode three backend classes (embedded ANN libs / metaengine engines / server DBs) so adapters never force a breaking SDK change and `dgo` never enters core `go.mod` |
...
| Quantify metaengine dep-tree delta (`go mod graph` before/after)         | 🔴 `TODO`    | Med    | M      | hardens CONTRA #1 of `docs/research/2026-09-15_metaengine-system-adoption.md` (report f.2) into numbers; owner 2026-09-16: run soon, not trigger-gated |
